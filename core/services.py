...
class ImportExportService:
    ...
    def export_month_report_pdf_sorted_by_amount(self, mois_id: int, filepath: Path) -> Result:
        """Crée un rapport PDF complet du mois avec les opérations triées par montant décroissant."""
        try:
            mois = self.db_manager.get_mois_by_id(mois_id)
            if not mois:
                return Result.error("Mois non trouvé pour le rapport PDF.")
            ...
            story = []
            story.append(Paragraph(f"Rapport du mois : {mois.nom} (trié par montant desc)", title_style))
            story.append(Paragraph(f"Date de génération : {datetime.datetime.now().strftime('%d/%m/%Y %H:%M')}", normal_style))
            story.append(Spacer(1, 8))
            ...
            story.append(Paragraph("Détails des opérations (triées par montant décroissant)", section_style))
            if depenses:
                rows = [["Date", "Nom", "Catégorie", "Montant", "Type", "Payé", "Fixe"]]
                for d in sorted(depenses, key=lambda x: x.montant if x.montant is not None else 0.0, reverse=True):
                    montant_style = ParagraphStyle(
                        'AmountCell',
                        parent=cell_style,
                        textColor=colors.HexColor('#16a34a') if d.est_credit else colors.HexColor('#111827'),
                    ...
                    rows.append([
                        Paragraph(d.date_depense or "", cell_style),
                        Paragraph(d.nom or "", cell_style),
                        Paragraph(d.categorie or "", cell_style),
                        Paragraph(f"{d.montant:.2f} €", montant_style),
                        Paragraph('Revenu' if d.est_credit else 'Dépense', cell_style),
                        Paragraph('Oui' if d.effectue else 'Non', cell_style),
                        Paragraph('Oui' if d.est_fixe else 'Non', cell_style),
            ...
            logger.error(f"Erreur inattendue lors de la génération du rapport PDF trié par montant: {e}")
            return Result.error(f"Une erreur inattendue est survenue lors de la génération du PDF: {e}")
